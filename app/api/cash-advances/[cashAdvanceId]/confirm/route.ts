...
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ cashAdvanceId: string }> },
) {
  ...
  if (session.user.role !== Role.AGENT) {
    return NextResponse.json(
      { message: "Unauthorized: Only agents can confirm receipt." },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    if (!cashAdvance) {
      return NextResponse.json(
        { message: "Cash advance not found or you are not the recipient" },
        { status: 404 },
      );
    }
    ...
    if (cashAdvance.status !== "PENDING") {
      return NextResponse.json(
        { message: `Cash advance is already ${cashAdvance.status}` },
        { status: 400 },
      );
    }
