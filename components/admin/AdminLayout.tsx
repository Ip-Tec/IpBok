...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/login");
    } else if (status === "authenticated" && session.user.role !== "SUPERADMIN" && session.user.role !== "SUPPORT") {
      router.push("/dashboard");
    }
  }, [status, session, router]);

  if (status === "loading") {
    return <div className="flex h-screen items-center justify-center bg-background">Loading Admin Console...</div>;
  }

  if (!session || (session.user.role !== "SUPERADMIN" && session.user.role !== "SUPPORT")) {
    return null;
  }
